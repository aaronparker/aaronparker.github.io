...
- **Title overlay on hero** — the post title, category label, and metadata bar are now rendered inside the hero image over a gradient overlay (`transparent → rgba(0,0,0,0.85)`), removing the visual gap between image and content; hero height increased to `clamp(360px, 48vw, 580px)` to accommodate the text comfortably; the image is inset with content-matching padding (`px-6 pt-6 lg:px-10`) and given rounded corners (`rounded-xl`) and a shadow; the attribution pin moves to the top-right corner to avoid overlap with the title text; posts without a hero image fall back to the original full header layout below the fold; the `sizes` attribute on the responsive `<source>` element updated to account for padding (`calc(100vw - 48px)` on mobile, `calc(100vw - 360px)` on desktop)
