...
- Simple Icons (GitHub, Bluesky, Ko-fi, Spotify) use `fill` — need `fill="currentColor"` on `<svg>` root
- RSS uses `stroke="currentColor"` / `fill="none"` (correct already)
- LinkedIn already had `fill="currentColor"`
- **TODO:** Fix `scripts/copy-icons.js` to inject `fill="currentColor"` on Simple Icons so it survives rebuilds
...
### Social Icon Colours (SVG fill)
GitHub, Bluesky, Ko-fi, Spotify SVGs are regenerated by `scripts/copy-icons.js` on every `npm run build`, wiping `fill="currentColor"`. Fix needed inside the script to inject the attribute on write. RSS and LinkedIn work correctly.

### TOC (Table of Contents)
TOC is populated by `toc.js` — verify it still works correctly after the layout refactors.

### `scripts/copy-icons.js` Fix
Add `fill="currentColor"` injection when writing Simple Icons SVGs so the attribute persists across builds:
```js
// Pseudocode — in copy-icons.js when writing Simple Icons
